    ...
    <header role='banner' className='absolute left-0 right-0 top-0 z-20 text-white'>
      <nav className='relative mx-auto flex max-w-screen-xl items-center gap-6 px-3 md:px-6 py-4'>
        ...
        <span className='z-20 ml-auto md:hidden'>
          <Hamburger color='#3730a3' label='Show menu' rounded toggled={isOpen} toggle={setOpen} />
        </span>
        <div
          className={`${
                ...
                return (
                  <React.Fragment key={`nav-${index}`}>
                    {nav.items.length > 1 ? (
                      ...
                      <li key={`nav-${index}`}>
                        <PrismicLink className={navStyles} field={nav.primary.link} activeClassName='text-indigo-700 dark:text-indigo-100'>
                          {nav.primary.label}
                        </PrismicLink>
                      </li>
                    )}
