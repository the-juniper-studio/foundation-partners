...
const PricingTable = ({ slice }) => {
  return (
    <section className={`component-pricingtable pt-10 relative`}>
      ...
      <div className='px-4 my-5 sm:text-center prose dark:prose-invert prose-lg lg:prose-xl max-w-screen-xl mx-auto'>
        <PrismicRichText field={slice.primary.title.richText} />
        <PrismicRichText field={slice.primary.description.richText} />
      </div>
      <FadeInStagger duration='.5'>
        <ul className='md:grid grid-cols-3 gap-6 md:mt-12 mt-3 space-y-3 md:space-y-0 max-w-screen-2xl mx-auto'>
          {slice.items.map((product, index) => {
            return (
              <li key={`product-${index}`}>
                <FadeIn x='0' y='50' className='flex flex-grow flex-col gap-6 md:gap-12 ease relative transform p-6 h-full bg-white dark:bg-white/10'>
