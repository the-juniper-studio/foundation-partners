    ...
    <li className='component-teamMember'>
      <FadeIn x='0' y='50' className='ease transform gap-6'>
        ...
        <div className='flex h-full flex-col justify-end sm:col-span-2'>
          <div>
            <div className='name inline-block px-6 text-center'>
              <h2>{teamMemberData.name}</h2>
              {teamMemberData.qualification && <small>{teamMemberData.qualification}</small>}
            </div>
            <div className='position px-6 py-1 inline-block font-semibold text-lg -top-1 relative -rotate-1'>{teamMemberData.position}</div>
            <div className='mt-3 flex flex-row flex-wrap gap-3 text-sm'>
              {contact?.linked_in && (
                <PrismicLink className='justify-items flex items-center hover:text-brandRust' href={contact.linked_in.url}>
                  <svg xmlns='http://www.w3.org/2000/svg' className='h-6 lg:h-8' fill='currentColor' viewBox='0 0 20 20' aria-hidden='true'>
                    <path
                      fillRule='evenodd'
              ...
              {contact?.email_address && (
                <PrismicLink className='justify-items flex items-center hover:text-brandRust' href={`mailto:${contact.email_address}`}>
                  <Email className='h-6 lg:h-8' />
                  <span className='sr-only whitespace-nowrap'>Email {contact.email_address}</span>
                </PrismicLink>
              )}
              {contact?.phone_number && (
                <PrismicLink className='justify-items flex items-center hover:text-brandRust' href={`tel:${contact.phone_number}`}>
                  <Phone className='h-6 lg:h-8' />
