    ...
    <li className='component-teamMember'>
      <FadeIn x='0' y='50' className='ease transform grid grid-cols-2 sm:grid-cols-3 items-start gap-6'>
        <div className='aspect-square w-full'>
          <GatsbyImage image={teamMemberData.image.gatsbyImageData} className='z-10 h-full w-full rounded-lg object-cover' alt={teamMemberData.image.alt || ''} />
        </div>
        <div className='flex h-full flex-col justify-end sm:col-span-2'>
          <div>
            <div className='flex items-baseline gap-2'>
              <h2 className=''>{teamMemberData.name}</h2>
              {teamMemberData.qualification && <small>{teamMemberData.qualification}</small>}
            </div>
            <div className='prose flex items-end dark:prose-invert lg:prose-lg xl:prose-xl'>{teamMemberData.position}</div>
            <div className='mt-3 flex flex-row flex-wrap gap-3 text-sm'>
              {contact?.linked_in && (
                <PrismicLink className='justify-items flex items-center gap-1' href={contact.linked_in.url}>
                  <svg xmlns='http://www.w3.org/2000/svg' className='h-4' fill='currentColor' viewBox='0 0 20 20' aria-hidden='true'>
                    <path
                      fillRule='evenodd'
              ...
              {contact?.email_address && (
                <PrismicLink className='justify-items flex items-center gap-1 no-underline hover:text-adonisBlue focus:text-adonisBlue' href={`mailto:${contact.email_address}`}>
                  <Email className='h-4' />
                  <span className='sr-only whitespace-nowrap'>Email {contact.email_address}</span>
                </PrismicLink>
              )}
              {contact?.phone_number && (
                <PrismicLink className='justify-items flex items-center gap-1 hover:text-adonisBlue focus:text-adonisBlue' href={`tel:${contact.phone_number}`}>
                  <Phone className='h-4' />
